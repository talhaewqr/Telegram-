...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'TECHX123');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                console.log(chalk.green.bold(`🎉 𓆩 ＳＩＬＶＥＲ  𓆪 online: ${kingbadboiNumber}`));
